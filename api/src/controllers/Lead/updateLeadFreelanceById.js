...
// Función para actualizar un lead freelancer por su ID en la base de datos
const updateLeadFreelanceById = async (id, updatedData) => {
  ...
  if (updatedData.dataLead.status === "Contratado") {
    ...
    const formattedTimeAgendaLlamada = dateAgendaLlamada.toISOString();
  ...
  if (updatedData.dataLead.status === "Contratado") {
    // Buscamos un registro que tenga el mismo 'emailApp' para borrarlo, ya que será añadido a un nuevo lead
